...
export function VariantPicker({ variants, onSelect }: Props) {
  ...
  const choosable = axes.filter((a) => a.values.length > 1).map((a) => a.name);
  ...
  function pick(name: string, value: string) {
    ...
    onSelect(
      remaining.length === 1
        ? remaining[0]
        : (remaining.find((v) => v.image_urls.length > 0) ?? null),
    );
  }
        ...
        <div key={axis.name} className="space-y-2">
          <div className="text-[13px] uppercase">
            ...
            {axis.values.length === 1 ? (
              <span className="ml-2 text-neutral-500">{axis.values[0]}</span>
            ) : (
              selection[axis.name] && (
            ...
            <div className="flex flex-wrap gap-[3px]">
              {axis.values.map((value) => {
                ...
                const pool = variants.filter(
                  (v) =>
                    matches(v, others) && matches(v, { [axis.name]: value }),
                );
          ...
          <span className="flex flex-wrap items-center gap-x-3">
            ...
            {chosen.price && (
              <span className="text-black">
                {formatMoney(chosen.price.price, chosen.price.currency)}
              </span>
            )}
            <Availability available={chosen.available} />
          </span>
        ) : (
          <span>{selectionHint(choosable, selection, candidates.length)}</span>
        )}
      </div>
    </div>
  );
...
function selectionHint(
  ...
  const missing = names.filter((name) => !selection[name]);
  if (missing.length === 0 && names.length === 0) {
    return "";
  }
  if (missing.length === names.length) {
    return `Select ${missing.map((n) => n.toLowerCase()).join(" and ")}`;
  }
  if (missing.length > 0) {
    return `Select ${missing.map((n) => n.toLowerCase()).join(" and ")} · ${remaining} available`;
  }
  return remaining === 0
  ...
  if (variants.length === 1) {
    ...
    return (
      <div className="flex flex-wrap items-center gap-x-3 text-[13px] uppercase text-neutral-500">
        ...
        {only.price && (
          <span className="text-black">
            {formatMoney(only.price.price, only.price.currency)}
          </span>
        )}
...
function Availability({ available }: { available: boolean | null }) {
  ...
  return (
    <span className={available ? "text-black" : "text-neutral-400"}>
